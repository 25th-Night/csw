...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Email Check
    if (signupEmailInput) {
        signupEmailInput.addEventListener('input', debounce(() => {
    ...
    // Phone Check
    if (signupPhoneInput) {
        signupPhoneInput.addEventListener('input', debounce(() => {
    ...
    if (signupPasswordInput) {
        signupPasswordInput.addEventListener('input', debounce(() => {
            formatPassword(signupPasswordInput.value, "signup-password");}, 500));
    }
    ...
    async function formatPassword(password, inputField) {
        ...
        if (password) {
            ...
            if (response.status === 200) {
                ...
                displayPermanentErrorMessage(!passwordCheck, inputField, "문자, 숫자, 특수문자 포함 8자리 이상으로 작성해주세요.")
            } else {
                console.log('API 요청 실패');
            }
        }
    ...
    signupPasswordConfirmInput.addEventListener('input', debounce(() => {
        formatPasswordConfirm(signupPasswordInput.value, signupPasswordConfirmInput.value, "signup-password-confirm");}, 500));
    ...
    async function formatPasswordConfirm(password, password_confirm, inputField) {
        ...
        if (password_confirm) {
            ...
            displayPermanentErrorMessage(password != password_confirm, inputField, "Password not matched.")
        }
    }
